      ...
      <Card className="max-w-4xl p-6 glass-card">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2"><Label>Department *</Label><Select value={formData.department} onValueChange={(v) => updateField('department', v)}><SelectTrigger><SelectValue placeholder="Select department" /></SelectTrigger><SelectContent><SelectItem value="NestOps">NestOps</SelectItem><SelectItem value="NestHQ">NestHQ</SelectItem><SelectItem value="NestTech">NestTech</SelectItem><SelectItem value="NestLabs">NestLabs</SelectItem><SelectItem value="Nest People">Nest People</SelectItem></SelectContent></Select></div>
            ...
            <div className="space-y-2"><Label>Joining Date *</Label><Input type="date" value={formData.joiningDate ? format(formData.joiningDate, 'yyyy-MM-dd') : ''} onChange={(e) => updateField('joiningDate', e.target.value ? new Date(e.target.value) : undefined)} required /></div>
            ...
            <div className="space-y-2"><Label>Status</Label><div className="flex items-center gap-3 h-10"><Switch checked={formData.status} onCheckedChange={(c) => updateField('status', c)} /><span className={formData.status ? 'text-primary font-medium' : 'text-muted-foreground'}>{formData.status ? 'Active' : 'Inactive'}</span></div></div>
          </div>
